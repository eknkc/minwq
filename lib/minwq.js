...
Queue.prototype.execScript = function (name, keys, args, next) {
  ...
  var cargs = [this.scripts[name].hash, keys.length].concat(keys).concat([Date.now()]).concat(args);

  this.client.evalsha(cargs, function (err, result) {
    if (err && /^NOSCRIPT/.test(err.message)) {
      cargs[0] = self.scripts[name].data
      return self.client.eval(cargs, next);
    }
