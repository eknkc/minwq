...
var Queue = function (options) {
  ...
  this.options.prefix = this.options.prefix || 'def';
  this.options.wqPrefix = 'mwq';
  this.options.client = this.options.client || redis.createClient();
  this.scripts = {}
...
Object.defineProperty(Queue.prototype, "queue", {
  get: function () {
    return [this.options.wqPrefix, this.options.prefix, opt.queue].join(':');
  },
  enumerable: true
});
...
Queue.prototype.push = function(opt, next) {
  ...
  this.execScript('push', [queue], [JSON.stringify(opt)], next);
}
...
Queue.prototype.pop = function(opt, next) {
  ...
  self.execScript('pop', [queue], [], function (err, data) {
    if (err) return next(err);
    if (!data) return next();
    ...
    next(null, {
      ...
      remove: function(next) {
        self.execScript('rem', [queue], [JSON.stringify({ id: data.id })], next);
      }
    });
  });
}
