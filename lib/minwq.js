...
module.exports = function(options) {
  ...
  function execScript(name, keys, args, next) {
    ...
    client.evalsha([scripts[name].hash, keys.length].concat(keys).concat(args), function (err, result) {
  ...
  return {
    push: function(opt, next) {
      ...
      execScript('push', [options.prefix + opt.queue], [JSON.stringify(opt.data), opt.unique, opt.priority, opt.delay, opt.ttr], next);
    },
